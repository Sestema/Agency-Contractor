...
Write-Host "Deploying admin-gateway to Supabase project $ProjectRef..." -ForegroundColor Cyan
...
try {
    npx --yes supabase@latest functions deploy admin-gateway `
        --project-ref $ProjectRef `
        --use-api

    if ($LASTEXITCODE -ne 0) {
        throw "admin-gateway deploy failed with exit code $LASTEXITCODE"
    }

    Write-Host ""
    Write-Host "Done. Restart AdminPanel and try saving Business again." -ForegroundColor Green
}
finally {
    Pop-Location
}
